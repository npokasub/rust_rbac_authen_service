...
impl<'a> UserRoleRepository<'a> {
  ...
  pub fn find_by_ids(&self, user_id: Uuid, role_id: Uuid) -> Result<UserRole, AppError> {
    ...
    let user_role = user_roles::table
      ...
      .first(&mut conn)
      .map_err(|e| {
        error!("UserRole with user_id={} and role_id={} not found: {}", user_id, role_id, e);
        AppError::NotFound(format!("UserRole with user_id={} and role_id={} not found", user_id, role_id))
      })?;
